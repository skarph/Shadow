...
We've just made another class, which is a child of the `Car` class. Despite not having any functions in the code above, it still has all of the functions of the `Car` class:
...
Update is called every frame, so it's where you'd put code which needs to be ran every frame. Simple, right?

Update is where you'd move something across the screen, or check if a button is pressed, or anything else which needs to be done every frame.
